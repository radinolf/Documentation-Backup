...
* [flat file](data-source.md#connections-to-flat-files) (Excel, CSV and XML) **(XML is Deprecated)**
...
### Data Source for XML files (Deprecated)
...
1. Select XML file as data source type
2. In the **Xml File connection** section, enter:

![](../../../.gitbook/assets/6)

* in **File Path on Client** the path, including the local XML file. The Browse button opens the window to select the file.
* in **File Path on** **Server** the path, including the XML file on the DAC server. The path must start from the name of the unit on the server, which must have visibility privileges on the folder where the file resides.
...
1. in _**name**_, the name you want to give the table
2. in _**path**_, identifies the element in the source file containing the information of interest
3. in _**dateFormat**_, the format of the dates in the table
4. in _**namespaces**_, the grouping of names under a single ID which is required to define the pertinence of the various TAGS used in the XML file

node \<column>, the individual columns in the table are defined, indicating:

1. in _**name**_, the name of the column
2. in _**path**_, the path where the datum is contained in the XML file relating to the column
3. in _**type**_, the data type

**Connection example to xml files**

The example proposes the configuration for the connection to a XML file containing information of the employees.

**Step 1:** XML Files Connection

![](<../../../.gitbook/assets/8 (3)>)

The source file consists of a series of elements empl:employee. which identify an employee.
...
* ![C:\c490439d88e47ebeed818ff9fb75957f](../../../.gitbook/assets/18)
