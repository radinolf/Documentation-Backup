...
Some DAC-DS services require a connection to external servers to be able to be executed, for example, sending notifications (see [Notification Designer](../../app-functionality/untitled.md))
...
* **Smtp-authentication-protocol:** allows specifying the authentication protocol to be used, choosing between non-encrypted options, SSL, or TLS. When TLS is selected as the protocol, **STARTTLS** support is automatically enabled, as it is integrated into the server configuration.
