...
### Positioning (Container Style) (Deprecated)
...
{% hint style="warning" %}
**Excaption:** the Row Container can have a % as height (height, min-height, max-height) only if the container has the flex property enabled
{% endhint %}
...
{% hint style="info" %}
The CSS File must be imported as a Resource via the [Resource Catalo](../../application/resource-catalog/)
{% endhint %}
