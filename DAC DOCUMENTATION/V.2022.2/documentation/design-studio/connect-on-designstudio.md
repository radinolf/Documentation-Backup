...
The DAC offers the possibility to automatically create users already enabled after authentication. Please see the section [**How to set "enabled" user automatically created**](connect-on-designstudio.md#how-to-set-enabled-user-automatically-created.)**.**
...
After selecting the system of authentication by OpenID Connect and define the type of authentication in Design Studio (For more detail se[ Server & Services](cloud/server-and-services.md) ) let's go to define who will be the System Administrator.
