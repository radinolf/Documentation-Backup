...
* **Select Login with Open Connect** . The authentication of Design Studio leverage the authentication service (OIDC) configured in the instance
* **Select Login As Administrator** if is the first Installation. Usually when you build the first installation you don't have a configured OIDC provider, and so in this case you have to connect with the built-in administrator account.
...
After selecting the system of authentication by OpenID Connect and define the type of authentication in Design Studio (For more detail se[ Server & Services](/broken/pages/-Mjcfc-EaiKt0AyAaMFz) ) let's go to define who will be the System Administrator.
