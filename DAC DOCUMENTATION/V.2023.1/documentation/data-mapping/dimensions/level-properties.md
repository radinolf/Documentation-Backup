...
* **level-global-name** Sets the gloabl name of the level. That global name is used to associate two or more levels. If a report level cannot be reached from aa cube, the system will check if there is alevel related to a dimension linked to the cube with the same global name (association made on the basis of the global name equality). If that is the case, that level will be used instead of the "missing" level.
