...
On postgresql environment system shows an exception during query validation of a sql report, this happens because `%metadata_schema%` is translated with `.` and db has a name with “`-`“ character.
