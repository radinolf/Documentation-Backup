...
* [weighted average](total-funtions.md#weighted-average-weighted-avg)
* [different functions](total-funtions.md#different-total-by-metric-composite), for each report metric
* [custom functions, using](total-funtions.md#different-total-by-metric-composite) Java interface classes, available in DAC
...
![](../../../../.gitbook/assets/0.png)Formatting for totals’ rows and/or columns is set from the Total group properties.
...
* [_Weighted Avg \[n\]_](total-funtions.md#weighted-average-weighted-avg) for the calculation of weighted averages
* [_Composite \[n\]_](total-funtions.md#different-total-by-metric-composite) to apply total functions specific to each report metric
* [_Custom Java \[n\]_](total-funtions.md#java-custom-total) to associate custom functions, previously developed in Java
...
_with \<entire-result> , , they are not aggregated by BU, so we carry out a sum of the BU for_ ![](<../../../../.gitbook/assets/16 (2).png>)_all quarters () and then we divide by the BU of all quarters ()_![](<../../../../.gitbook/assets/17 (3).png>)_)_
...
The ![](../../../../.gitbook/assets/37.png) button of the totals command bar opens the _**Custom Total**_ window where it is possible to select a series of functions grouped by topic:

* Mathematical Functions for mathematical formulas
* Report Reference&#x73;**:** for the functions to reference the data in the report
* SumFunctions for sum
* Formatting Functions for functions that format numeric values
* Conversion Functions for functions that convert data, used in complex operations.
...
The system enables the bar to determine the new total, where the title is inserted in the Title section and clicking on the ![](../../../../.gitbook/assets/40.png) button we access the window for the selection of functions.
...
_Finally we click on the_ ![](../../../../.gitbook/assets/42.png) _button to save the formula and add the custom total row to the report: the system calculates the minimum between the two values of the selected cells, highlighted by the formatting of the inserted total._
...
_For example, if you select the column containing the description of the products \[COLS$\_ID(B) ], this function returns the ID code for each individual product._![](../../../../.gitbook/assets/44.png)
