...
* [Direct filters](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Filtri_Diretti_1) where the condition is expressed on the dimensional levels, used to limit the data on the analysis lines (dimensions);
* [Range filters](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Filtri_Range) where the condition is expressed on the values of the metrics
...
DAC also defines the filters exclusively used by the single report.

You can use a [parameter in the condition](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Parametric_filter) in Filters. In these cases, the condition is applied after you have valued the parameter.
...
**Data Model>>**>>**Direct Filters management**, or the ![](<../../../.gitbook/assets/0 (11).png>) button on the tool bar.
...
The [filter condition](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Toc210196212) may be expressed compared to the dimensional levels of the application. When typing the formula, you are supported by a [Content Assist](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Content_Assist_su).
...
![](<../../../.gitbook/assets/12 (7).png>)![](<../../../.gitbook/assets/13 (7).png>)
...
Range filters define conditions on the metrics, already defined with the [Metrics Management](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Metriche) module.

Filters are then associated with the reports, which apply the conditions defined to the set of data as a whole.

The dimensioned range filters apply the filter condition on the data previously [partitioned for one or more dimensional levels](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Partizionamento_per_livelli); on this partitioning it is possible to [apply additional filters, of the direct type](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Filtri_diretti_sul), to refine the filter criterion further.

A range filter is created in the window opened from the menu item:

**Data Model>>Range Filter**

or from the toolbar, using the ![](<../../../.gitbook/assets/16 (5).png>) button.
...
Select the dimensional levels for the [data partitioning](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Hlt265834075) in the _Dimensions_ folder.

In the _Direct Filter_ folder, select the [direct filters](/broken/pages/-MfHt0F2wlKyy6VAhAlY#_Filtri_diretti_sul) to be applied to the partitioned data.
...
To start the User Filter editor from the tool bar select the button ![](<../../../.gitbook/assets/image (155).png>)
...
In the example User Filter was created which limits, for all the users of the **Regional User** group ,  the display of the data only relating to the status of competence (\[<#Action\_Status\_Id/>@ID]=1 ).
